...
function renderBank(){
    ...
    h += '<span style="flex:1"></span><button class="btn btn-sm btn-gold" id="bank-run" disabled>Run selected (0)</button></div>';
    TOPICS.forEach(function(t){
...
function renderQuestion(){
    ...
    function ready(){ var a=cur(); var has = p.type==='ma' ? (a&&a.length) : (p.type==='ne' ? !!a : a!=null); var b=$('q-check')||$('q-next'); if(!b) return; var gateOK = !(learn&&!sess.revealed) || (($('gate-txt')||{}).value||'').trim().length>=10; b.disabled = !(has && sess.conf[p.id] && gateOK); }
...
function renderPTest(){
    ...
    h += '<span style="flex:1"></span><button class="btn btn-green" id="pt-start"'+(n?'':' disabled')+'>&#9654; Start test ('+n+')</button></div>';
    TOPICS.forEach(function(t){
        var ps = PROBLEMS.filter(function(p){ return p.topic===t; });
